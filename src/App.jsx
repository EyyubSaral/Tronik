...
export default function App() {
  ...
  /* Challenge
	...
	Bu synth klavyede önemli bir şey henüz çalışmıyor: Tuşlar. Göreviniz, kullanıcının tuşları mouse tuşu ya da bilgisayar klavyesi ile aşağıdaki şekilde oynatmasına izin vermektir:  
		...
		3. Nesnenin diğer tüm özellikleri ve synthKeys state array'deki diğer tüm nesneler, state her güncellendiğinde korunmalıdır. Yalnızca hedeflenen nesnenin
    --- active özelliği ve (varsa) keyPressed özelliği değiştirilmelidir. 
...
*/
  ...
  const handleMouseDown = (e) => {
    ...
    setSynthKeys((prev) =>
      prev.map((key) => (key.keyName === note ? { ...key, active: true } : key))
    );
  };
  ...
  const handleMouseUp = (e) => {
    ...
    setSynthKeys((prev) =>
      prev.map((key) =>
        key.keyName === note ? { ...key, active: false } : key
      )
    );
  };
  ...
  const handleKeyUp = (e) => {
    ...
    setSynthKeys((prev) =>
      prev.map((key) =>
        key.computerKey === keyReleased
          ? { ...key, active: false, keyPressed: false }
          : key
      )
    );
  };
